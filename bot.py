...
class Bot:
	...
	def tweet(self,irtsi=None,at=None): 
		...
		if at and irtsi:
			status = "@"+at+" "+status
		try:
			self.poster.statuses.update(status=status,in_reply_to_status_id=irtsi)
	...
	def replies(self):
		...
		replies = [tweet['in_reply_to_status_id'] for tweet in self.poster.statuses.user_timeline() if tweet['in_reply_to_status_id'] != None]
		try: 
			self.last_id_replied = replies[0]
		except IndexError:
			self.last_id_replied = 0
		results = []
		results = self.twitter.search(q="@"+self.handle,since_id=self.last_id_replied)['results']
